...
export default function StudentJobsPage() {
  ...
  const fetchJobs = async (pageNum = 1, reset = false) => {
    try {
      ...
      const res = await fetch(`${(process.env.NEXT_PUBLIC_API_URL ? (process.env.NEXT_PUBLIC_API_URL.endsWith('/api') ? process.env.NEXT_PUBLIC_API_URL : process.env.NEXT_PUBLIC_API_URL + '/api') : 'https://campusbridge-e4cv.onrender.com/api')}/jobs?${query}`, {
        headers: {
