...
function CreateWebinarPageContent() {
  ...
  const fetchWebinarDetails = async () => {
    setFetching(true);
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || (process.env.NEXT_PUBLIC_API_URL || 'http://localhost:5000/api')}/webinars/alumni`, {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const url = editId 
        ? `${process.env.NEXT_PUBLIC_API_URL || (process.env.NEXT_PUBLIC_API_URL || 'http://localhost:5000/api')}/webinars/${editId}` 
        : (process.env.NEXT_PUBLIC_API_URL || (process.env.NEXT_PUBLIC_API_URL || 'http://localhost:5000/api')) + '/webinars';
