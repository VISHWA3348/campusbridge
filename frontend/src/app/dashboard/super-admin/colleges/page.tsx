...
export default function CollegesPage() {
  ...
  const handleToggleInviteCode = async (id: number) => {
    try {
      ...
      await fetch(`${(typeof window !== 'undefined' && window.location.hostname !== 'localhost' && window.location.hostname !== '127.0.0.1' ? 'https://campusbridge-e4cv.onrender.com/api' : (process.env.NEXT_PUBLIC_API_URL && process.env.NEXT_PUBLIC_API_URL !== '/api' ? process.env.NEXT_PUBLIC_API_URL : 'https://campusbridge-e4cv.onrender.com/api'))}/super-admin/colleges/${id}/toggle-invite-code`, {
        method: 'PATCH',
  ...
  const handleRegenerateInviteCode = async (id: number) => {
    ...
    try {
      const token = localStorage.getItem('token');
      await fetch(`${(typeof window !== 'undefined' && window.location.hostname !== 'localhost' && window.location.hostname !== '127.0.0.1' ? 'https://campusbridge-e4cv.onrender.com/api' : (process.env.NEXT_PUBLIC_API_URL && process.env.NEXT_PUBLIC_API_URL !== '/api' ? process.env.NEXT_PUBLIC_API_URL : 'https://campusbridge-e4cv.onrender.com/api'))}/admin/colleges/${id}/regenerate-invite-code`, {
        method: 'PATCH',
        headers: { 'Authorization': `Bearer ${token}` }
      });
      loadColleges();
  ...
  const handleOpenManageCodes = async (college: any) => {
    ...
    try {
      const token = localStorage.getItem('token');
      const res = await fetch(`${(typeof window !== 'undefined' && window.location.hostname !== 'localhost' && window.location.hostname !== '127.0.0.1' ? 'https://campusbridge-e4cv.onrender.com/api' : (process.env.NEXT_PUBLIC_API_URL && process.env.NEXT_PUBLIC_API_URL !== '/api' ? process.env.NEXT_PUBLIC_API_URL : 'https://campusbridge-e4cv.onrender.com/api'))}/admin/signup-codes?collegeId=${college.id}`, {
        headers: { 'Authorization': `Bearer ${token}` }
      });
  ...
  const handleToggleCodeStatus = async (codeId: number, currentStatus: string) => {
    try {
      ...
      await fetch(`${(typeof window !== 'undefined' && window.location.hostname !== 'localhost' && window.location.hostname !== '127.0.0.1' ? 'https://campusbridge-e4cv.onrender.com/api' : (process.env.NEXT_PUBLIC_API_URL && process.env.NEXT_PUBLIC_API_URL !== '/api' ? process.env.NEXT_PUBLIC_API_URL : 'https://campusbridge-e4cv.onrender.com/api'))}/admin/signup-codes/${codeId}`, {
        method: 'PUT',
        headers: { 
