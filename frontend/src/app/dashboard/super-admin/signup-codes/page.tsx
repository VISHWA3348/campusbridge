...
export default function SignupCodesPage() {
  ...
  const fetchData = async () => {
    try {
      const [codesRes, collegesRes] = await Promise.all([
        fetch((process.env.NEXT_PUBLIC_API_URL || (process.env.NEXT_PUBLIC_API_URL || 'https://campusbridge-e4cv.onrender.com/api')) + '/admin/signup-codes', {
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      const res = await fetch((process.env.NEXT_PUBLIC_API_URL || (process.env.NEXT_PUBLIC_API_URL || 'https://campusbridge-e4cv.onrender.com/api')) + '/admin/signup-codes', {
        method: 'POST',
        headers: { 
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      await fetch(`${process.env.NEXT_PUBLIC_API_URL || (process.env.NEXT_PUBLIC_API_URL || 'https://campusbridge-e4cv.onrender.com/api')}/admin/signup-codes/${id}`, {
        method: 'DELETE',
        headers: { 'Authorization': `Bearer ${localStorage.getItem('token')}` }
      });
      fetchData();
  ...
  const toggleStatus = async (code: any) => {
    ...
    try {
      await fetch(`${process.env.NEXT_PUBLIC_API_URL || (process.env.NEXT_PUBLIC_API_URL || 'https://campusbridge-e4cv.onrender.com/api')}/admin/signup-codes/${code.id}`, {
        method: 'PUT',
        headers: { 
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${localStorage.getItem('token')}`
