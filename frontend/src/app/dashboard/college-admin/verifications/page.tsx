...
export default function VerificationsPage() {
  ...
  const fetchPending = async () => {
    try {
      ...
      const res = await fetch((process.env.NEXT_PUBLIC_API_URL ? (process.env.NEXT_PUBLIC_API_URL.endsWith('/api') ? process.env.NEXT_PUBLIC_API_URL : process.env.NEXT_PUBLIC_API_URL + '/api') : 'https://campusbridge-e4cv.onrender.com/api') + '/college-admin/verifications/pending', {
  ...
  const handleApprove = async (userId: number) => {
    ...
    try {
      const token = localStorage.getItem('token');
      const res = await fetch(`${(process.env.NEXT_PUBLIC_API_URL ? (process.env.NEXT_PUBLIC_API_URL.endsWith('/api') ? process.env.NEXT_PUBLIC_API_URL : process.env.NEXT_PUBLIC_API_URL + '/api') : 'https://campusbridge-e4cv.onrender.com/api')}/college-admin/verifications/approve/${userId}`, {
        method: 'POST',
        headers: { 'Authorization': `Bearer ${token}` }
      });
      if (res.ok) {
  ...
  const handleReject = async () => {
    ...
    try {
      const token = localStorage.getItem('token');
      const res = await fetch(`${(process.env.NEXT_PUBLIC_API_URL ? (process.env.NEXT_PUBLIC_API_URL.endsWith('/api') ? process.env.NEXT_PUBLIC_API_URL : process.env.NEXT_PUBLIC_API_URL + '/api') : 'https://campusbridge-e4cv.onrender.com/api')}/college-admin/verifications/reject/${selectedUser.id}`, {
        method: 'POST',
        headers: { 
