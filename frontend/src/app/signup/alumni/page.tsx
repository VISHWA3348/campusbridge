...
export default function AlumniSignup() {
  ...
  const validateInviteCode = async (code: string) => {
    ...
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || (process.env.NEXT_PUBLIC_API_URL || 'http://localhost:5000/api')}/auth/validate-invite-code?code=${code}&role=ALUMNI`);
