...
export default function StudentSignup() {
  ...
  const validateInviteCode = async (code: string) => {
    ...
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || (process.env.NEXT_PUBLIC_API_URL || 'http://localhost:5000/api')}/auth/validate-invite-code?code=${code}&role=STUDENT`);
  ...
  const handleResendOTP = async () => {
    try {
      const res = await fetch((process.env.NEXT_PUBLIC_API_URL || (process.env.NEXT_PUBLIC_API_URL || 'http://localhost:5000/api')) + '/auth/resend-otp', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
